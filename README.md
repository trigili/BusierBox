...
For target builds, selecting `doom` builds the static BusierBox `doom-ascii` provider through Buildroot. Set `BB_DOOM_WAD_PATH` to a local legally usable `.wad`; BusierBox stages that data file and does not download or bundle game data by default. Target packaging fails visibly if the staged `doom-ascii` engine is not static.
