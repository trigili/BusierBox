...
LINE_COMMAND_HELP_TOPICS = {
    ...
    "files": {
        ...
        "entries": [
            ...
            ("release stage SELECTOR", "stage a release artifact"),
    ...
    "events": {
        ...
        "entries": [
            ...
            ("limit 50", "inside events: show the last 50 matching events"),
            ("service file-service", "inside events: filter by service"),
            ("event upload", "inside events: filter by event name"),
            ("level warning", "inside events: filter by level"),
            ("target my-router", "inside events: filter by target id or label"),
            ("since 2h", "inside events: show events from the last N seconds/minutes/hours/days"),
            ("events service=file-service", "filter by service from any context"),
    ...
    "probe": {
        ...
        "entries": [
            ("probe",                                   "enter probe context and show the target-side command"),
            ("start",                                   "inside probe: start the probe.sh listener on port 22207"),
            ("queue",                                   "inside probe: queue probe command for selected target's next phone-home"),
            ("delivery",                                "inside probe: show wget, curl, and raw nc delivery commands"),
            ("paste",                                   "inside probe: print a serial/admin-shell heredoc"),
            ("paste base64",                            "inside probe: print a base64 paste wrapper"),
            ("script",                                  "inside probe: print the raw generated probe.sh script"),
            ("results",                                 "inside probe: show received probe results"),
            ("config [N]",                              "inside probe: generate config from a probe result"),
            ("config --write-config FILE",              "inside probe: generate and save config"),
            ("clear [N|--all]",                         "inside probe: remove stale probe results"),
            ("serve [start]",                           "inside probe: stage the matching release binary"),
            ("probe start|queue|results",               "run probe actions from any context"),
        ],
        "notes": [
        ...
        "examples": [
            ...
            "results",
            "config --write-config config.json",
            "serve start",
        ],
    },
    "survey": {
    ...
    "build": {
        ...
        "entries": [
            ...
            ("verbose", "inside build: show config options and examples"),
            ("build -v", "show build config options and examples from any context"),
            ("set KEY|NUMBER VALUE", "inside build: set a build config field"),
            ("unset KEY|NUMBER", "inside build: clear a build config field"),
            ("build set KEY|NUMBER VALUE", "set a build config field from any context"),
            ("build unset KEY|NUMBER", "clear a build config field from any context"),
