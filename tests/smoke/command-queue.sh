...
scripts/busierbox-server --config "$queued_cfg" --json-status | python3 -c 'import json,sys; d=json.load(sys.stdin); q=d["command_queue"]; summary=d["summary"]; api=d["api_collections"]["command_queue_commands"]; commands=q["commands"]; assert len(commands) == 1; command_id=commands[0]["id"]; assert summary["command_queue_execution_decision_counts"]["rejected"] == 1; assert summary["command_queue_queue_policy_enabled_counts"]["true"] == 1; assert summary["command_queue_queue_policy_valid_counts"]["true"] == 1; assert summary["command_queue_queue_policy_execution_mode_counts"]["metadata-only"] == 1; assert summary["command_queue_queue_policy_allowed_commands_counts"]["busierbox-only"] == 1; assert summary["command_queue_delivery_policy_enabled_counts"]["true"] == 1; assert summary["command_queue_delivery_policy_valid_counts"]["true"] == 1; assert summary["command_queue_delivery_policy_execution_mode_counts"]["metadata-only"] == 1; assert q["commands_by_execution_decision"]["rejected"][0]["id"] == command_id; assert q["commands_by_queue_policy_enabled"]["true"][0]["id"] == command_id; assert q["commands_by_queue_policy_valid"]["true"][0]["id"] == command_id; assert q["commands_by_queue_policy_execution_mode"]["metadata-only"][0]["id"] == command_id; assert q["commands_by_queue_policy_allowed_commands"]["busierbox-only"][0]["id"] == command_id; assert q["commands_by_delivery_policy_enabled"]["true"][0]["id"] == command_id; assert q["commands_by_delivery_policy_valid"]["true"][0]["id"] == command_id; assert q["commands_by_delivery_policy_execution_mode"]["metadata-only"][0]["id"] == command_id; assert d["commands_by_queue_policy_execution_mode"]["metadata-only"][0]["id"] == command_id; assert d["commands_by_delivery_policy_valid"]["true"][0]["id"] == command_id; assert "commands_by_execution_decision" in api["indexes"]; assert "commands_by_queue_policy_execution_mode" in api["indexes"]; assert "commands_by_delivery_policy_valid" in api["indexes"]'
