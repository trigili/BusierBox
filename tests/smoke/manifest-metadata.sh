...
if command_queue.get("mode_records_by_delivery_supported", {}).get("false") != [0, 1, 2, 3, 4]:
    raise SystemExit("manifest-metadata: command queue delivery-support index missing")
if command_queue.get("mode_records_by_result_upload_supported", {}).get("true") != [0, 1, 2, 3, 4]:
    raise SystemExit("manifest-metadata: command queue result-upload-support index missing")
...
if command_queue.get("mode_records_by_operator_supplied_command_execution", {}).get("false") != [0, 1, 2, 3, 4]:
    raise SystemExit("manifest-metadata: command queue operator-supplied execution index missing")
if (mode_summary.get("mode_count") != 5 or
        ...
        mode_summary.get("active_control_channel_mode_count") != 0 or
...
if (mode_api.get("count") != 5 or
        ...
        "mode_records_by_target_polling_supported" not in mode_api.get("indexes", []) or
        "mode_records_by_delivery_supported" not in mode_api.get("indexes", []) or
        "mode_records_by_result_upload_supported" not in mode_api.get("indexes", []) or
        "mode_records_by_operator_supplied_command_execution" not in mode_api.get("indexes", [])):
