...
printf '%s\n' "$bringup_json" | python3 -c 'import json,sys; d=json.load(sys.stdin); api=d["api_collections"]; files=d["run_file_records"]; assert d["command_record_summary"]["total_count"] == len(d["next_command_records"]); assert api["next_command_records"]["count"] == len(d["next_command_records"]); assert "next_command_records_by_request" in api["next_command_records"]["indexes"]; assert api["next_command_records"]["summary_key"] == "command_record_summary.total_count"; assert api["run_file_records"]["count"] == len(files); assert "run_files_by_exists" in api["run_file_records"]["indexes"]; assert "run_files_by_name" in api["run_file_records"]["indexes"]; assert "run_files_by_path" in api["run_file_records"]["indexes"]; assert api["run_file_records"]["summary_key"] == "run_file_summary.total_count"; assert len(files) == d["run_file_summary"]["total_count"]; assert d["run_files_by_name"]["recommended_config"]["path"] == d["recommended_config"]; assert d["run_files_by_path"][d["recommended_config"]]["name"] == "recommended_config"; assert d["run_files_by_expected_kind"]["dir"][0]["name"] == "run_dir"; assert d["run_files_by_exists"]["yes"]'
...
printf '%s\n' "$release_json" | python3 -c 'import json,sys; d=json.load(sys.stdin); files=d["run_files"]; reality=d["reality_summary"]; providers=d["selected_tool_provider_status"]; wads=d["selected_doom_wads"]; summary=d["selected_artifact_summary"]; reverse=summary["reverse_access"]; queue=summary["command_queue"]; api=reality["api_collections"]["checks"]; assert d["compatibility"]["label"] == "likely"; assert d["max_compatibility"] == "likely"; assert d["selected_artifact"].endswith("busierbox-mipsel-full"); assert providers["gdbserver"]["overall"] == "found"; assert wads[0]["filename"] == "doom.wad"; assert wads[0]["size"] == 9; assert reverse["session_policy"] == "reconnect"; assert reverse["session_policy_summary"]["fresh_session_on_reconnect"] is True; assert reverse["session_policy_summary"]["session_resume_supported"] is False; assert queue["enabled"] == "no"; assert queue["daemon_stop_supported"] is True; assert queue["executes_commands"] is False; assert d["reality_json"]; assert reality["check_count"] == 2; assert reality["checks_by_name"]["tmp_noexec"] == [1]; assert reality["checks_by_type"]["constraint"] == [1]; assert reality["checks_by_status"]["pass"] == [0,1]; assert reality["constraints"]["tmp_noexec"] is True; assert api["count"] == 2; assert api["count_summary_key"] == "summary.check_count"; assert api["summary_key"] == "summary.check_count"; assert "artifact-config set" in d["generated_trailer_override_command"]; assert d["recommendation"]["config"]["BB_RUNTIME_MODE"] == "extract"; assert d["recommendation"]["facts"]["reality"]["tmp_noexec_detected"] is True; assert any("reality-test detected /tmp noexec" in w for w in d["recommendation"]["warnings"]); assert files["release_find_json"]["exists"] is True; assert files["selected_artifact"]["path"] == d["selected_artifact"]; assert files["selected_artifact"]["exists"] is True; assert files["reality_json"]["exists"] is True'
