...
printf '%s\n' "$bringup_json" | python3 -c 'import json,sys; d=json.load(sys.stdin); s=d["command_record_summary"]; assert d["status"] == "pass"; assert d["recommended_target_preset"].endswith("smoke-bringup.json"); assert d["staged_fetch_command"]; assert "./busierbox reality-test --json" in d["next_target_commands"]; assert d["safety_boundary"]["network_autorun"] is False; assert d["safety_boundary"]["target_must_run_fetch"] is True; assert d["safety_boundary"]["hidden_control_channel"] is False; assert any(r["command"] == d["staged_fetch_command"] and r["side"] == "target" and r["network"] is True and r["requires_explicit_target_action"] is True and r["executes_operator_supplied_commands"] is False for r in d["next_target_command_records"]); assert all(r["executes_on_target"] is False for r in d["next_operator_command_records"]); assert s["operator_count"] == len(d["next_operator_command_records"]); assert s["target_count"] == len(d["next_target_command_records"]); assert s["target_network_count"] == 1; assert s["target_executes_operator_supplied_commands_count"] == 0; assert s["operator_executes_on_target_count"] == 0'
