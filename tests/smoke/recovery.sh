...
case ${bb##*/} in
    grit*) ;;
    *)
        ln -s "$(cd "$(dirname "$bb")" && pwd)/$(basename "$bb")" "$tmp/grit"
        bb="$tmp/grit"
        ;;
esac
...
"$bb" persistence install --method rc-local --dry-run --root "$tmp/root" --name grit_recovery >"$tmp/dry-run"
grep -q 'Would install persistence method=rc-local' "$tmp/dry-run"
grep -q 'Action: status-only' "$tmp/dry-run"
grep -q 'Generated command: /usr/bin/grit_recovery persistence status' "$tmp/dry-run"
grep -q 'Would backup existing hook' "$tmp/dry-run"
"$bb" persistence install --method rc-local --dry-run --json --root "$tmp/root" --name grit_recovery >"$tmp/dry-run.json"
...
assert data["name"] == "grit_recovery"
assert data["action"] == "status-only"
assert data["action_category"] == "status"
assert data["generated_command"] == "/usr/bin/grit_recovery persistence status"
assert data["paths"]["hook"] == f"{root}/etc/rc.local"
assert data["paths"]["binary"] == f"{root}/usr/bin/grit_recovery"
...
if "$bb" persistence install --method rc-local --root "$tmp/root" --name grit_recovery 2>"$tmp/err"; then
    printf '%s\n' "recovery: install without --apply unexpectedly succeeded" >&2
    exit 1
fi
grep -q 'require --apply' "$tmp/err"

if "$bb" persistence install --method rc-local --apply --name grit_recovery 2>"$tmp/real-root-install.err"; then
...
if "$bb" persistence uninstall --method rc-local --apply --name grit_recovery 2>"$tmp/real-root-uninstall.err"; then
...
"$bb" persistence install --method rc-local --apply --root "$tmp/root" --name grit_recovery >/dev/null
test -x "$tmp/root/usr/bin/grit_recovery"
grep -q 'BEGIN GRIT RECOVERY grit_recovery' "$tmp/root/etc/rc.local"
grep -q 'action=status-only' "$tmp/root/etc/rc.local"
grep -q 'persistence status' "$tmp/root/etc/rc.local"
ls "$tmp/root/etc"/rc.local.grit.bak.* >/dev/null
"$bb" persistence status --root "$tmp/root" --name grit_recovery >"$tmp/status"
grep -q 'installed_method=rc-local' "$tmp/status"
grep -q 'installed_kind=rc.local marked block' "$tmp/status"
grep -q "installed_binary=$tmp/root/usr/bin/grit_recovery" "$tmp/status"
...
"$bb" persistence status --json --root "$tmp/root" --name grit_recovery >"$tmp/status.json"
...
for needle in [
    "recovery binary method=rc-local action=status-only name=grit_recovery",
    "recovery marked hook method=rc-local action=status-only name=grit_recovery",
]:
    if needle not in details:
        raise SystemExit(f"missing recovery ledger metadata: {needle}")
PY
"$tmp/root/usr/bin/grit_recovery" persistence status --root "$tmp/root" --name grit_recovery >"$tmp/copied-status"
grep -q 'installed_method=rc-local' "$tmp/copied-status"
"$tmp/root/usr/bin/grit_recovery" recovery status --root "$tmp/root" --name grit_recovery >"$tmp/copied-recovery-status"
grep -q 'installed_method=rc-local' "$tmp/copied-recovery-status"

"$bb" recovery install --method rcS --dry-run --root "$tmp/root" --name grit_recovery >"$tmp/rcs-dry-run"
...
"$bb" persistence install --method rc-local --action evidence-push --dry-run --root "$tmp/root" --name grit_recovery >"$tmp/evidence-dry-run"
grep -q 'Action: evidence-push' "$tmp/evidence-dry-run"
grep -q 'Generated command: /usr/bin/grit_recovery evidence push --quiet' "$tmp/evidence-dry-run"
"$bb" persistence install --method rc-local --action evidence-push --target-id target-recovery --target-label "Recovery Router" --target-alias lab-router --dry-run --json --root "$tmp/root" --name grit_recovery >"$tmp/evidence-target-dry-run.json"
...
PY
GRIT_TARGET_ID=env-recovery GRIT_TARGET_LABEL="Env Router" "$bb" persistence install --method rc-local --action evidence-push --dry-run --json --root "$tmp/root" --name grit_recovery >"$tmp/evidence-env-target-dry-run.json"
...
PY
"$bb" persistence install --method rc-local --action evidence-push --target-id target-recovery --target-label "Recovery Router" --target-alias lab-router --apply --root "$tmp/root" --name grit_recovery >/dev/null
...
"$bb" persistence status --root "$tmp/root" --name grit_recovery >"$tmp/evidence-target-status"
...
"$bb" persistence status --json --root "$tmp/root" --name grit_recovery >"$tmp/evidence-target-status.json"
...
PY
"$bb" persistence uninstall --method rc-local --apply --root "$tmp/root" --name grit_recovery >/dev/null
"$bb" persistence install --method rc-local --action evidence-push --apply --root "$tmp/root" --name grit_recovery >/dev/null
grep -q 'action=evidence-push' "$tmp/root/etc/rc.local"
grep -q '/usr/bin/grit_recovery evidence push --quiet' "$tmp/root/etc/rc.local"
"$bb" persistence status --json --root "$tmp/root" --name grit_recovery >"$tmp/evidence-status.json"
...
assert item["starts_rshell_after_evidence"] is False
assert item["executes_operator_supplied_command"] is False
assert item["generated_command"] == "/usr/bin/grit_recovery evidence push --quiet"
assert item["binary_present"] is True
PY
"$bb" persistence uninstall --method rc-local --apply --root "$tmp/root" --name grit_recovery >/dev/null

"$bb" persistence install --method rc-local --action evidence-then-rshell --apply --root "$tmp/root" --name grit_recovery >/dev/null
grep -q 'action=evidence-then-rshell' "$tmp/root/etc/rc.local"
grep -q '/usr/bin/grit_recovery evidence push --quiet && /usr/bin/grit_recovery rshell start' "$tmp/root/etc/rc.local"
"$bb" persistence status --root "$tmp/root" --name grit_recovery >"$tmp/evidence-rshell-status"
grep -q 'installed_action=evidence-then-rshell' "$tmp/evidence-rshell-status"
grep -q 'installed_command=/usr/bin/grit_recovery evidence push --quiet && /usr/bin/grit_recovery rshell start' "$tmp/evidence-rshell-status"
"$bb" persistence status --json --root "$tmp/root" --name grit_recovery >"$tmp/evidence-rshell-status.json"
...
"$bb" persistence install --method rc-local --action dmesg-push --dry-run --root "$tmp/root" --name grit_recovery >"$tmp/dmesg-dry-run"
grep -q 'Action: dmesg-push' "$tmp/dmesg-dry-run"
grep -q 'grit_dmesg_dir=' "$tmp/dmesg-dry-run"
grep -q 'dmesg >"$grit_dmesg"' "$tmp/dmesg-dry-run"
grep -q -- '--dest grit_recovery-dmesg.txt' "$tmp/dmesg-dry-run"
"$bb" persistence install --method rc-local --action dmesg-push --dry-run --json --root "$tmp/root" --name grit_recovery >"$tmp/dmesg-dry-run.json"
...
assert data["action_semantics"]["executes_operator_supplied_command"] is False
assert data["action_semantics"]["command_queue_enabled"] is False
assert "grit_dmesg_dir=" in data["generated_command"]
assert "dmesg >\"$grit_dmesg\"" in data["generated_command"]
assert "--dest grit_recovery-dmesg.txt" in data["generated_command"]
PY
"$bb" persistence install --method rc-local --action dmesg-push --apply --root "$tmp/root" --name grit_recovery >/dev/null
grep -q 'action=dmesg-push' "$tmp/root/etc/rc.local"
grep -q 'rm -f "$grit_dmesg"' "$tmp/root/etc/rc.local"
"$bb" persistence status --json --root "$tmp/root" --name grit_recovery >"$tmp/dmesg-status.json"
...
assert item["starts_rshell"] is False
assert item["executes_operator_supplied_command"] is False
assert "grit_dmesg_dir=" in item["generated_command"]
assert "dmesg >\"$grit_dmesg\"" in item["generated_command"]
assert "--dest grit_recovery-dmesg.txt" in item["generated_command"]
PY
"$bb" persistence uninstall --method rc-local --apply --root "$tmp/root" --name grit_recovery >/dev/null

"$bb" persistence uninstall --method rc-local --dry-run --json --root "$tmp/root" --name grit_recovery >"$tmp/uninstall-dry-run.json"
...
PY
"$bb" persistence uninstall --method rc-local --apply --root "$tmp/root" --name grit_recovery >/dev/null
test ! -e "$tmp/root/usr/bin/grit_recovery"
if grep -q 'BEGIN GRIT RECOVERY grit_recovery' "$tmp/root/etc/rc.local"; then
...
"$bb" persistence install --method cron-reboot --action command --apply --root "$tmp/root" --name grit_recovery -- 'grit rshell start' >/dev/null
grep -q 'action=command' "$tmp/root/etc/crontabs/root"
grep -q 'grit rshell start' "$tmp/root/etc/crontabs/root"
"$bb" persistence status --json --root "$tmp/root" --name grit_recovery >"$tmp/cron-status.json"
...
assert any(item["method"] == "cron-reboot" and item["action"] == "command" and item["action_category"] == "command" and item["executes_operator_supplied_command"] is True and item["generated_command"] == "grit rshell start" for item in data["installations"])
PY
"$bb" persistence uninstall --method cron-reboot --apply --root "$tmp/root" --name grit_recovery >/dev/null

printf '%s\n' '#!/bin/sh' 'echo recovery-script' >"$tmp/script.sh"
"$bb" persistence install --method rc-local --action script --file "$tmp/script.sh" --dry-run --root "$tmp/root" --name grit_recovery >"$tmp/script-dry-run"
grep -q "Would copy script: $tmp/root/usr/bin/grit_recovery.recovery.sh from $tmp/script.sh" "$tmp/script-dry-run"
grep -q 'Generated command: /usr/bin/grit_recovery.recovery.sh' "$tmp/script-dry-run"
"$bb" persistence install --method rc-local --action script --file "$tmp/script.sh" --apply --root "$tmp/root" --name grit_recovery >/dev/null
test -x "$tmp/root/usr/bin/grit_recovery.recovery.sh"
grep -q 'action=script' "$tmp/root/etc/rc.local"
grep -q '/usr/bin/grit_recovery.recovery.sh' "$tmp/root/etc/rc.local"
"$bb" persistence status --root "$tmp/root" --name grit_recovery >"$tmp/script-status"
grep -q 'installed_action=script' "$tmp/script-status"
grep -q "installed_script=$tmp/root/usr/bin/grit_recovery.recovery.sh" "$tmp/script-status"
grep -q 'installed_script_present=yes' "$tmp/script-status"
"$bb" persistence status --json --root "$tmp/root" --name grit_recovery >"$tmp/script-status.json"
...
assert item["generated_command"] == "/usr/bin/grit_recovery.recovery.sh"
assert item["script_path"] == f"{root}/usr/bin/grit_recovery.recovery.sh"
...
data = json.load(open(sys.argv[1], encoding="utf-8"))
details = "\n".join(item.get("detail", "") for item in data.get("entries", []))
if "recovery script method=rc-local action=script name=grit_recovery source=" not in details:
    raise SystemExit("missing recovery script ledger metadata")
PY
"$bb" persistence uninstall --method rc-local --apply --root "$tmp/root" --name grit_recovery >/dev/null
test ! -e "$tmp/root/usr/bin/grit_recovery.recovery.sh"
