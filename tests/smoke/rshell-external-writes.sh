...
prev_root_copy = text.rfind('GRIT_RSHELL_AUTHKEYS_MODE, "root-copy"', 0, idx)
prev_root_merge = text.rfind('GRIT_RSHELL_AUTHKEYS_MODE, "root-merge"', 0, idx)
prev_disabled = text.rfind('GRIT_RSHELL_AUTHKEYS_MODE, "disabled"', 0, idx)
prev_payload_home = text.rfind('GRIT_RSHELL_AUTHKEYS_MODE, "payload-home"', 0, idx)
...
root_copy_start = text.find('GRIT_RSHELL_AUTHKEYS_MODE, "root-copy"')
root_merge_start = text.find('GRIT_RSHELL_AUTHKEYS_MODE, "root-merge"')
hostkey_start = text.find('GRIT_RSHELL_GENERATE_HOSTKEY_IF_MISSING', root_merge_start)
...
for needle in [
    ...
    'grit_ledger write /root/.ssh/authorized_keys external root-copy',
]:
...
for needle in [
    'append_rshell_ledger_setup(cmd, sizeof(cmd));',
    'bak=/root/.ssh/authorized_keys.grit.bak.$$',
    'grit_ledger backup',
    'grit_ledger modify /root/.ssh/authorized_keys external root-merge',
]:
