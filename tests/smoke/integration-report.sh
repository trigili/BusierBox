...
grep -q '^release_self_test_status=pass release=fixture-release artifacts=1 tuples=1 devices=1 command_queue_enabled=0 command_queue_exec=0$' local/tmp.integration-report.out
...
scripts/integration-report "$new" --json | python3 -c 'import json,sys; d=json.load(sys.stdin); e=d["operator_events"]; r=d["release_self_test"]; assert e["total_count"] == 3; assert e["invalid_count"] == 0; assert e["first_event_at"] == "2026-05-26T12:00:00Z"; assert e["latest_event_at"] == "2026-05-26T12:00:05Z"; assert e["event_counts"]["upload_complete"] == 1; assert e["level_counts"]["info"] == 3; assert e["service_counts"]["file-service"] == 3; assert e["service_event_counts"]["file-service:upload_complete"] == 1; assert e["remote_counts"]["192.0.2.10:40100"] == 1; assert e["session_counts"]["20260526-file-service"] == 1; assert e["recent"][-1]["event"] == "shutdown"; assert r["valid"] is True; assert r["status"] == "pass"; assert r["release_name"] == "fixture-release"; assert r["command_queue_enabled_count"] == 0; assert r["command_queue_execution_supported_count"] == 0'
