...
- `scripts/`: copied `grit-console`, its `gritlib` modules, `artifact-config`,
  `bringup`, `config-from-survey`, plus wrapper helpers, release self-test,
  index, and finder tools.
...
lifecycle summary. The line-oriented workbench service-start actions print the
equivalent `--transport SERVICE` command and service-start events record the
...
local files.
In the line-oriented workbench, plain file stage, list, and unstage actions
...
`events_by_event_detail_target_identity_confidence`, and
...
target identity audit lookups such as `http-header`, `explicit`,
`upload_complete:http-header`, and `file-service:explicit`,
poll-setting lookups such as `poll`, `11`, or `exponential`,
...
behavior, target-execution behavior, source format, set-command availability,
set-command kind, safety boundary, reverse access relevance, command queue
relevance, control-like behavior, and explicit operator-choice requirements so
