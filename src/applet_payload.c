...
static void print_clean_json(int dry_run, int include_external, int ledger, int external_applied)
{
    ...
    json_string_payload(stdout, bb_ledger_path(path, sizeof(path)));
...
int applet_recovery_main(int argc, char **argv)
{
    ...
    if (!strcmp(cmd, "status")) {
        ...
        if (json) {
            ...
            json_string_payload(stdout, root);
            fputs(",\"name\":", stdout);
            json_string_payload(stdout, name);
        ...
        for (j = 0; j < sizeof(recovery_methods) / sizeof(recovery_methods[0]); j++) {
            ...
            if (recovery_status_one(root, &recovery_methods[j], name, action, sizeof(action), generated, sizeof(generated))) {
                ...
                if (json) {
                    ...
                    json_string_payload(stdout, recovery_methods[j].name);
                    fputs(",\"path\":", stdout); json_string_payload(stdout, path);
                    fputs(",\"action\":", stdout); json_string_payload(stdout, action[0] ? action : "unknown");
                    fputs(",\"generated_command\":", stdout); json_string_payload(stdout, generated);
                    fputc('}', stdout);
                } else {
...
{
    char payload[PATH_MAX], archive[PATH_MAX], ledger[PATH_MAX];
    struct embedded_payload ep;
    int have_payload = candidate_payload(payload, sizeof(payload)) == 0;
    int have_embedded = get_embedded_payload(&ep) == 0;
    int have_archive = archive_path(archive, sizeof(archive)) == 0;
    ...
    if (json) {
        ...
        fputs(",", stdout);
        {
            ...
            json_string_payload(stdout, p);
        }
        fputs("],\"would_modify\":[", stdout);
        json_string_payload(stdout, BB_RUNTIME_ROOT);
        fputs("],\"would_remove\":[],\"would_start\":[],\"would_connect\":[],\"requires_external_writes\":false", stdout);
        fputs(",\"runtime_root\":", stdout); json_string_payload(stdout, BB_RUNTIME_ROOT);
        fputs(",\"fallback_root\":", stdout); json_string_payload(stdout, BB_RUNTIME_FALLBACK_ROOT);
        printf(",\"fallback_enabled\":%s", !strcmp(BB_RUNTIME_ALLOW_FALLBACK_ROOT, "yes") ? "true" : "false");
        fputs(",\"cleanup_ledger_path\":", stdout); {
...
static void plan_print_rshell(int json)
{
    ...
    if (json) {
        ...
        json_string_payload(stdout, guard);
        fputs(",", stdout);
        json_string_payload(stdout, log_path);
        fputs("],\"would_modify\":[", stdout);
        json_string_payload(stdout, guard);
        fputs("],\"would_remove\":[],\"would_start\":[", stdout);
        json_string_payload(stdout, BB_RSHELL_TRANSPORT);
        fputs("],\"would_connect\":[", stdout);
        json_string_payload(stdout, server);
        fputs("],\"requires_external_writes\":false", stdout);
        fputs(",\"runtime_root\":", stdout); json_string_payload(stdout, BB_RUNTIME_ROOT);
        fputs(",\"transport\":", stdout); json_string_payload(stdout, BB_RSHELL_TRANSPORT);
        fputs(",\"encryption\":", stdout); json_string_payload(stdout, BB_RSHELL_ENCRYPTION);
        fputs(",\"run_mode\":", stdout); json_string_payload(stdout, BB_RSHELL_RUN_MODE);
        fputs(",\"shell_provider\":", stdout); json_string_payload(stdout, BB_RSHELL_SHELL_PROVIDER);
        fputs(",\"operator_host\":", stdout); json_string_payload(stdout, BB_OPERATOR_SERVER_HOST);
        fputs(",\"expected_transport_behavior\":", stdout); json_string_payload(stdout, connect);
...
static int plan_recovery_install(int argc, char **argv, int json)
{
    ...
    if (json) {
        ...
        json_string_payload(stdout, bin);
        fputs("],\"would_modify\":[", stdout);
        json_string_payload(stdout, hook);
        fputs("],\"would_remove\":[],\"would_start\":[", stdout);
        json_string_payload(stdout, generated);
        fputs("],\"would_connect\":[", stdout);
        if (!strcmp(action, "rshell"))
            json_string_payload(stdout, BB_OPERATOR_SERVER_HOST);
        fputs("],\"requires_external_writes\":", stdout);
        printf("%s", !strcmp(root, "/") ? "true" : "false");
        fputs(",\"root\":", stdout); json_string_payload(stdout, root);
        fputs(",\"method\":", stdout); json_string_payload(stdout, m->name);
        fputs(",\"action\":", stdout); json_string_payload(stdout, action);
        fputs(",\"hook_path\":", stdout); json_string_payload(stdout, hook);
        fputs(",\"binary_path\":", stdout); json_string_payload(stdout, bin);
        fputs(",\"generated_command\":", stdout); json_string_payload(stdout, generated);
